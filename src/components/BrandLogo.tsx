...
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  const sizeMap = {
    xs: { box: 'w-7 h-7', icon: 'w-4 h-4', text: 'text-sm', sub: 'text-[9px]', img: 'w-7 h-7' },
    sm: { box: 'w-8 h-8', icon: 'w-5 h-5', text: 'text-base', sub: 'text-[10px]', img: 'w-8 h-8' },
    md: { box: 'w-10 h-10', icon: 'w-6 h-6', text: 'text-lg sm:text-xl', sub: 'text-[10px]', img: 'w-10 h-10' },
    lg: { box: 'w-12 h-12', icon: 'w-7 h-7', text: 'text-xl sm:text-2xl', sub: 'text-xs', img: 'w-12 h-12' },
    xl: { box: 'w-16 h-16', icon: 'w-10 h-10', text: 'text-2xl sm:text-3xl', sub: 'text-xs', img: 'w-16 h-16' }
  };
  ...
  return (
    ...
    >
      ...
      <div className={`relative ${config.box} rounded-xl p-[1.5px] bg-gradient-to-br from-blue-500 via-cyan-400 to-indigo-600 shadow-lg shadow-cyan-500/25 group-hover:shadow-cyan-400/50 group-hover:scale-105 transition-all duration-300 shrink-0`}>
        <div className="w-full h-full bg-[#081120] rounded-[10px] flex items-center justify-center overflow-hidden">
          {!imageError ? (
            <img
              src={logoSrc}
            ...
            /* Cyber Vector Monogram Fallback */
            <svg 
              viewBox="0 0 512 512" 
              className="w-full h-full p-1 drop-shadow-[0_0_8px_rgba(0,240,255,0.6)]" 
              fill="none" 
